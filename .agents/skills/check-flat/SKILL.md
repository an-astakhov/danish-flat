...
7. If routing cannot be verified, use `&mdash;` for the affected travel field and continue adding the listing. Do not estimate from a map image, search snippet, or straight-line distance.

## Field rules

- **Ad:** Link to the canonical listing URL, labeled `Ad`.
- **Map / address:** Link to Google Maps, labeled with the exact full address. Use `&mdash;` if no usable address is stated.
- **Nearest metro:** State the verified metro station name and walking-route distance, for example `Enghave Brygge &mdash; 650 m`. Make the entire value a Markdown link to Google Maps walking directions from the resolved origin to that station. Prefix the distance with `~` when the origin address is imprecise.
...
- **Available:** Output an exact ISO date (`YYYY-MM-DD`) when the ad supplies a month and day. If the year is omitted, compare that month and day with today's date in the user's or workspace timezone: use the current year when the resulting date is today or in the future, otherwise use the next year. For example, on `2026-08-14`, `Oct 1` becomes `2026-10-01` and `Feb 1` becomes `2027-02-01`. Apply this rollover only to availability, not to viewing dates, deadlines, or other dates. Use `Now`, `By agreement`, or `&mdash;` only when that is all the ad states. Do not flag the inferred availability year in `Note AI`.
- **sqm:** Numeric living area followed by `m&sup2;`, for example `72 m&sup2;`.
...
- **Outdoor:** Record only explicit facts, such as `Private balcony`, `Private terrace`, `Shared garden`, multiple features joined by `; `, `No`, or `&mdash;`. Do not turn access to a courtyard into a private garden.
...
After updating `flats.md`, mention that the user can invoke `$sync-flat-dashboard` to import the Markdown tracker into the interactive dashboard. Do not run that skill automatically unless the user also asks for a dashboard sync.

## Encoding safety

- Save `flats.md` as UTF-8.
- Use ASCII-safe HTML entities for generated punctuation that must render as symbols: write `&mdash;` for an em dash and `&sup2;` for square metres when needed. Do not write incorrectly decoded text.
